# Ludo Game: decision-making algorithm
...
**Clarifications:** (added recently)
* If one player's two tokens have reached the end, the following rolls of this player in the turns list will not change the state of the player anymore.  For example, if player A's state is already ['E', 'E'] at some point, if you read ('A', 5) in the turns list, it will not move player A's tokens.
* The general game rule says that when there is only one player who hasn't finished the game, this round of the game is over.  Since we didn't require you to implement this rule,  if there is only one player that hasn't finished the game, you could let this player continue to move if the turns list still has movement steps for this player.
* When a token is kicked back to its home, the total steps of this token should be reset to -1.
...
4. Move the token that is further away from the finishing square
   * **Furthest token away from the final space moves**
